...
export default function ActivityLog() {
  ...
  const getEventBadge = (eventType: string) => {
    ...
    const badgeClass =
      badges[eventType as keyof typeof badges] ||
      "bg-gray-500 text-white px-2 py-1 rounded-full text-xs font-medium";

    return <span className={badgeClass}>{eventType.replace("_", " ")}</span>;
  };
      ...
      <div className="p-6">
        ...
        >
          ...
          <div className="pl-6 pr-6 pt-3">
            <div className="flex items-center justify-between">
              ...
              >
                <Link href={`/${restaurant.tenantId}/global-activity-log`}>
                  <Button
                    variant="outline"
                    className="flex items-center gap-2 hover:bg-green-50 hover:border-green-500 transition-all duration-200"
                  >
            ...
            >
              Monitor your restaurant's performance and activity trends for{" "}
              {restaurant.name}
            </motion.p>
          </div>
