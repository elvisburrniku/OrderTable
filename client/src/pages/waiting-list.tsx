...
export default function WaitingList() {
  ...
  const { data: waitingList = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/restaurants/${restaurant?.id}/waiting-list`,
      );
  ...
  const updateEntryMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: number; updates: any }) => {
      const response = await fetch(
        `/api/restaurants/${restaurant?.id}/waiting-list/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const filteredWaitingList =
    (waitingList as any)?.filter((item: any) => {
        ...
        <div className="bg-white rounded-lg shadow">
                      ...
                      <td className="py-3 px-4">
                        ...
                        >
                          <SelectTrigger className="w-32">
                            <SelectValue>
                              <span
                                className={`px-2 py-1 text-xs rounded-full ${getStatusColor(item.status || "waiting")}`}
                              >
                                {item.status || "waiting"}
                              </span>
                            </SelectValue>
          ...
          <div className="p-4 border-t flex items-center justify-between">
            <div className="text-sm text-gray-600">
              ...
              {filteredWaitingList.reduce(
                (sum: number, item: any) => sum + (item.guestCount || 0),
                0,
              )}{" "}
              total guests
            </div>
