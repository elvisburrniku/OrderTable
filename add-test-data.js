...
async function addTestData() {
  try {
    ...
    const { restaurant_id: restaurantId, tenant_id: tenantId } =
    ...
    if (tablesResult.rows.length === 0) {
      console.error(
        "No tables found for this restaurant. Please create tables first.",
      );
      return;
    }
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      customerBatch.push(
        `(${restaurantId}, ${tenantId}, '${customer.name}', '${customer.email}', '${customer.phone}', ${customer.total_bookings}, ${customer.is_walk_in}, ${customer.notes ? `'${customer.notes}'` : "NULL"}, '${customer.last_visit.toISOString()}')`,
      );
    ...
    console.log(
      `Retrieved ${insertedCustomers.rows.length} customers for booking generation`,
    );
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      bookingBatch.push(
        `(${restaurantId}, ${tenantId}, ${Math.random() < 0.8 ? table.id : "NULL"}, ${customer.id}, '${customer.name}', '${customer.email}', '${customer.phone}', ${guestCount}, '${bookingDate.toISOString()}', '${startTime}', '${endTime}', '${status}', '${source}', ${notes ? `'${notes}'` : "NULL"})`,
      );
    ...
    console.log(
      `Updated ${bookingsWithoutHash.rows.length} booking management hashes`,
    );
    ...
    console.log(
      `Total customers in restaurant ${restaurantId}: ${finalCustomerCount.rows[0].count}`,
    );
    console.log(
      `Total bookings in restaurant ${restaurantId}: ${finalBookingCount.rows[0].count}`,
    );
