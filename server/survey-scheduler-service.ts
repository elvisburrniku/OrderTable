...
export class SurveySchedulerService {
  ...
  async scheduleSurvey(
    ...
    try {
      ...
      const booking = await this.storage.getBookingById(
        tenantId,
        restaurantId,
        bookingId,
      );
      ...
      const bookingEndDateTime = new Date(
        `${booking.bookingDate.toISOString().split("T")[0]}T${booking.endTime}:00`,
      );
      const scheduledFor = new Date(
        bookingEndDateTime.getTime() + hoursAfterBooking * 60 * 60 * 1000,
      );
      ...
      if (!customerEmail && !customerPhone) {
        console.log(
          `Survey scheduler: No contact information for booking ${bookingId}`,
        );
        return;
      }

      // Generate survey link
      const surveyLink = `${process.env.BASE_URL || "http://localhost:5000"}/survey/${responseToken}`;
      ...
      console.log(
        `Survey scheduler: Survey scheduled for booking ${bookingId} at ${scheduledFor.toISOString()}`,
      );
    } catch (error) {
  ...
  async processPendingSurveys(): Promise<void> {
    try {
      if (!this.storage.db) {
        console.log(
          "Survey scheduler: Database not available - skipping survey processing",
        );
        return;
      }
      ...
      console.log(
        `Survey scheduler: Found ${pendingSurveys.length} pending surveys to process`,
      );
      ...
      console.error(
        "Survey scheduler: Error processing pending surveys:",
        error,
      );
    }
  }
  ...
  private async sendSurvey(survey: any): Promise<void> {
    try {
      ...
      if (
        (survey.deliveryMethod === "email" ||
          survey.deliveryMethod === "both") &&
        ...
        try {
          if (this.emailService) {
            ...
            emailSent = true;
            console.log(
              `Survey scheduler: Email sent to ${survey.customerEmail} for booking ${survey.bookingId}`,
            );
          } else {
      ...
      if (
        (survey.deliveryMethod === "sms" || survey.deliveryMethod === "both") &&
        ...
        try {
          if (this.smsService) {
            ...
            smsSent = true;
            console.log(
              `Survey scheduler: SMS sent to ${survey.customerPhone} for booking ${survey.bookingId}`,
            );
          } else {
      ...
      const isSuccess =
        (survey.deliveryMethod === "email" && emailSent) ||
