  ...
  try {
    ...
    switch (event.type) {
      case "payment_intent.succeeded":
        ...
        if (paymentIntent.metadata?.bookingId === 'guest_booking' || paymentIntent.metadata?.type === 'guest_booking') {
            ...
            try {
              ...
              if (booking) {
                ...
                if (booking.status === 'waiting_payment') {
                  ...
                  console.log(`Updating booking ${booking.id} status from waiting_payment to confirmed`);
                }
                
                await storage.updateBooking(booking.id, updateData);
                console.log(`Updated booking ${booking.id} with payment status: paid`);
