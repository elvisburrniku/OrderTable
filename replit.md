...
- June 17, 2025. Fixed guest feedback form rating collection - resolved issue where public feedback endpoint was missing aggregation logic, causing guest submissions to show null ratings. Updated public API endpoint to include same individual question response processing as authenticated endpoint, ensuring QR code feedback submissions now properly display ratings and NPS scores in management dashboard.
- June 17, 2025. Enhanced feedback system with default overall rating step and improved dashboard display - added final 5-star rating step to guest feedback forms, updated feedback dashboard to display proper 5-star visual ratings instead of text, implemented NPS score display from 0-10 with color coding (green 9-10, yellow 7-8, red below 7), configured overall rating to take priority over individual question ratings, and completed full QR code to dashboard workflow with enhanced visual presentation.
- June 17, 2025. Fixed rating logic validation issue - resolved "8/5" rating display problem by implementing proper rating constraints (1-5 range), converted existing invalid ratings in database to proper 5-star scale, added frontend and backend validation to prevent 10-point scale ratings from being stored as main ratings, ensuring logical consistency between individual question ratings and overall star ratings.
- June 17, 2025. Standardized all feedback questions to 5-star rating system - converted circular 0-10 rating scale questions to consistent 5-star system with interactive star icons, updated guest feedback form to use star ratings for all question types, fixed rating priority logic to ensure overall 5-star rating takes precedence over individual question ratings, and enhanced visual consistency across feedback interface.
- June 17, 2025. Completed feedback system refinements - removed NPS Score display from feedback responses list and details modal for cleaner interface, added delete functionality to feedback responses with proper backend API endpoint and confirmation dialogs, enhanced "New" badge to only display for feedback created on current date, maintained unified 5-star rating system across all displays, and implemented complete CRUD operations for feedback management with proper tenant validation.
- June 17, 2025. Fixed conflict resolution system database error - resolved "-Infinity" database error in booking conflict resolution by adding proper validation for table capacities, implementing fallback default capacity when no valid tables exist, adding guest count validation to prevent division by zero, and ensuring minimum table calculation values to prevent database constraint violations during split party resolution.
- June 17, 2025. Implemented comprehensive activity logging system - added detailed logging for all user and guest activities including authentication events (login/logout/registration), booking operations (create/update/cancel), guest feedback submissions via both authenticated and QR code forms, table and room management, profile updates, and password changes. Activity logs capture IP addresses, user agents, detailed event descriptions, source tracking (manual/guest_form/qr_code), and comprehensive metadata for security and operational monitoring.
- June 17, 2025. Created global activity log system for all projects - implemented comprehensive global activity logging that shows activities across all restaurants in a tenant account, added database and memory storage methods for tenant-wide activity retrieval, created complete frontend interface with filtering by restaurant/event type/source, added navigation between restaurant-specific and global activity views, and enabled full visibility into system-wide user and guest interactions with restaurant context and detailed metadata.
- June 17, 2025. Implemented comprehensive automatic activity logging middleware - created ActivityLogger class that automatically captures all user actions across the entire project including login/logout/registration, booking operations, customer management, table/room management, feedback operations, restaurant settings, staff management, menu operations, integration events, and billing activities. Every API call now generates detailed activity logs with user information, IP addresses, user agents, timestamps, response times, and comprehensive metadata for complete audit trails and security monitoring.
- June 18, 2025. Enhanced Payment Setups system with exact design implementation - updated form layout to precisely match user-provided design specifications including proper method selection cards with green highlighting for "Capture amount", conditional form displays for single vs multiple pricing, styled input fields with gray backgrounds, proper checkbox styling for residual payments, and correctly positioned Save button. Form now includes all required fields: Name, Method (Capture/Reserve/No-show fee), Type (Prepayment dropdown), Price options (One price/Multiple prices), dynamic pricing table with add/remove functionality, currency selection, cancellation notice, description with language support, and complete backend API integration with database storage.
- June 20, 2025. Successfully migrated from Replit Agent to standard Replit environment - completed full migration with proper security practices and client/server separation. Enhanced bookings table with advanced filtering system including collapsible filter controls, search functionality by name/email, status and source dropdown filters, pagination with customizable results per page (10/20/50), navigation controls, color-coded status badges, and download CSV functionality. Table design matches user specifications with proper green accent colors and professional styling.
- June 21, 2025. Implemented consistent premium table designs across bookings and customers pages - applied matching clean header styling with gray backgrounds and compact spacing, enhanced customers page with Customer ID, Phone, Total Bookings columns, added active filters display with colored badges and count indicators, implemented 7 rows default pagination, created centered pagination layout with "1-7 of 20" format matching user specifications, removed redundant elements for cleaner interface, and ensured visual consistency between both table interfaces with professional styling and intuitive navigation controls.
```